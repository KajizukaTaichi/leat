        ...
        match self {
            ...
            Expr::Let(name, value, after_expr) => match *name.clone() {
                ...
                Expr::Call(name, arg) => {
                    ...
                    let func = Lambda::UserDefined(arg, value.to_owned(), env.clone());
                    let func = Box::new(Expr::Literal(Value::Lambda(func)));
                    Expr::Let(name, func, after_expr.clone()).reduct(env)
                }
                _ => Err(LeatError::InvalidBind(*name.clone())),
            },
            Expr::If(cond, then, els) => Ok(Expr::If(
                Box::new(cond.reduct(env)?),
                Box::new(then.reduct(env)?),
                Box::new(els.reduct(env)?),
            )),

            Expr::Try(risky, callback) => Ok(Expr::Try(
